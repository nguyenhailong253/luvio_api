...
{ read -d'\n' DB_NAME DB_SCHEMA; } < <(aws secretsmanager get-secret-value --secret-id luvio-api-secrets --region ap-southeast-2 --output json | jq -r '.SecretString' | jq -r '.dbname, .dbschema')
